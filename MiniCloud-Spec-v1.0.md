...
| Serving Authorization | Leader 在确认多数派后签发/刷新的短期服务许可，绑定 Worker Session、Assignment Generation 和 Policy Digest；只用于限制新调用，不进入 Raft。 |
...
| DEC-011 | 有界可用性优先但不承诺严格单活 | 控制面短暂故障时，旧 Worker 在 Serving Authorization 内继续 Ready Replica；新 Leader 通过更高 Generation/Epoch 收敛，允许最长受 TTL 限制的副本重叠。 |
| DEC-012 | 异步数据面使用成熟耐久队列 | Trigger 配置属于 Raft；Invocation、Fire Record、载荷、租约和结果属于独立队列/任务存储。参考 NATS JetStream 适配以持久事件流为权威、查询状态为可重建投影，并满足 ASY-013，不自研消息系统。 |
...
| Observability | 指标、结构化日志、Trace 和审计事件 | 默认记录敏感请求正文 |

## 9. 核心数据模型

所有持久控制对象必须包含 `id`、`namespace`、`created_at`、`updated_at` 和单调递增的 `resource_revision`。时间以 UTC 存储并使用 RFC 3339 表示。
...
| admission_epoch | 发布、禁用或删除时递增；Invocation 固定记录其被接收时的 Epoch。 |
...
| targets | `{version_id, admission_epoch, deployment_generation, weight_basis_points}` 的完整映射。 |
| affinity_source | `request_id`、`idempotency_key` 或显式 Header。 |
| hash_version/salt_id | 版本化的稳定哈希算法与平台 Salt 标识，不暴露 Salt 值。 |
| enabled | 可原子禁用入口。 |
...
| allocated | 当前承诺资源。 |
| state | `Joining`、`Ready`、`Suspect`、`Unavailable`、`Draining`、`Drained`、`Removed`；短期活性为 Observation，Drain/Removed 意图属于控制状态。 |
...
| desired_state | `Assigned` 或 `Cancelled`；实际 Pulling/Ready/Lost 由 Worker Observation 提供。 |
| mode | `normal` 或仅处理删除前已确认异步任务的 `drain-only`。 |
| raft_origin | 创建该 Intent 的 Term、Log Index 和 Command ID。 |

### 9.7 EndpointSnapshot

| 字段 | 要求 |
| --- | --- |
| function_id | EndpointSnapshot 的 Function 作用域；组合 Watch 可承载多个独立快照。 |
| discovery_epoch | 当前 Leader 在本 Term 提交并应用的 Leadership Barrier Log Index；跨 Leader 单调。 |
| sequence | 只在同一 Discovery Epoch 内从 1 单调递增。 |
| route_revision | 该 EndpointSnapshot 对应的 Route Revision。 |
| checksum | 完整规范化快照校验和。 |
...
EndpointSnapshot 是由当前 Leader 根据已提交 Assignment Intent 和 Worker Ready Observation 生成的版本化发现快照，不是新的 Raft 业务对象。Gateway 只能从完整快照增加 Endpoint；可根据本地被动/主动健康检查临时移除 Endpoint，但不得在无控制面授权时自行新增或迁移。
...
| function | 目标 Function。 |
| payload_template | 可选、有大小限制。 |
...
| Capability/Trigger 配置 | Raft FSM | 是 | 不含 Secret 值或事件载荷。 |
| Raft Member/持久 Operation ID | Raft FSM/Stable Store | 是 | 用于成员关系和控制写去重。 |
...
| EndpointSnapshot/LKG | Leader Watch + Gateway 内存；磁盘仅诊断 | 否 | 完整版本化发现快照，带 Epoch、Sequence、Checksum；重启后必须 Full Sync。 |
| Serving Authorization | Leader/Worker 短期内存 | 否 | Worker 以本地单调接收点执行 TTL；Leader 无 Quorum 时不能刷新。 |
...
| Cron Fire Record/Event Delivery | Durable Queue/Task Store | 否 | Trigger 配置在 Raft，投递记录在数据面。 |
| 日志、指标、Trace | Observability Backend | 否 | 不作为业务状态真相。 |

任何新增数据类型在实现前必须先归类到此表。不得仅为“方便复制”而把高频数据加入 Raft。

### 9.11 Revision、Cursor、Epoch 与 Digest 语义
...
| `endpoint_sequence` | 单个 Discovery Epoch | EndpointSnapshot 在本 Epoch 内的序号。 |
| `state_digest` | 一个 Applied Index 的权威 FSM | 对规范化 Function、Version、Route、Deployment Desired State、Assignment Intent、Worker Registry、Trigger 配置和成员配置计算；明确排除 Heartbeat、Observation、EndpointSnapshot、Gateway ACK、派生 Condition、Artifact 和 Invocation。 |
...
| ID | 优先级 | 需求 | 验收要点 |
| --- | --- | --- | --- |
| FN-001 | P0 | 平台必须支持创建、查询、列出、禁用和删除 Function。 | 重名返回冲突；禁用后新调用返回稳定错误。 |
| FN-002 | P0 | 发布必须创建新的不可变 Version，不允许覆盖 Ready Version。 | 相同制品可产生不同版本，但版本配置不可原地变更。 |
| FN-003 | P0 | 版本必须经过摘要、格式、ABI、Import/Feature 和编译验证后才能进入 Ready。 | 非 WASM、损坏模块、缺少 `_start`、未知 Import/Feature 被拒绝；支持但无资源授权的 WASI 调用按 Capability 处理。 |
| FN-004 | P0 | Failed Version 必须保留可诊断状态，但不得进入路由。 | 查询可见错误码；Gateway 永不选择该版本。 |
| FN-005 | P0 | Version 进入 `Deleting` 时递增 Admission Epoch 并禁止新 Route/Trigger/普通 Assignment；旧 LKG 在窗口内可能接受旧 Epoch 调用。启用异步后，可为已确认旧 Epoch 任务保留/创建 `drain-only Assignment`；Admission LKG 结束且 Task Store 连续观察无非终态任务后才提交 `Tombstoned`。 | FSM Apply 不查询 Queue；Controller 以 Observation 驱动后续命令，不声称跨存储原子事务。 |
| FN-006 | P1 | 平台应该支持弃用 Version；弃用阻止新增路由，但不破坏已有路由。 | 弃用与删除语义不同。 |
| FN-007 | P0 | 所有资源写操作必须支持预期 `resource_revision`，防止丢失更新。 | 修订不匹配返回 `revision_conflict`。 |
| FN-008 | P0 | Version Artifact GC 宽限必须严格大于 `max(route_history_ttl, route_max_stale + async_task_max_age + deletion_safety_margin)`，并等待已知 Assignment 清理且不存在 Pinned Route Revision。 | 已确认/迟到异步任务和显式回滚保留都不会失去 Artifact。 |
| FN-009 | P0 | Function 删除必须先禁用入口、等待全部 Version Tombstone 与 Assignment 清理，再进入 Tombstoned。 | 删除过程可查询、可重入且不会接受新调用。 |
| FN-010 | P1 | 启用异步后，删除排空必须等待 Task Store Watermark 越过 Admission Cutoff，并在 `deletion_quiescence` 内连续观察零非终态旧 Epoch 任务；单次可能滞后的查询不能触发 Tombstone。 | 投影延迟不会导致提前 GC。 |
...
| ART-006 | P0 | Tombstoned Version 的 Artifact 只可在保守宽限后回收；宽限覆盖 Route 历史、Gateway LKG、Deployment/Assignment 和异步任务可能引用窗口。 | Core 删除闭环不依赖 P1；回收可审计。 |
| ART-007 | P1 | Worker 应使用有界 LRU/近似 LRU 编译缓存，并暴露命中率与淘汰原因。 | 达到容量后不导致无界磁盘或内存增长。 |
| ART-008 | P0 | 发布事务顺序必须是临时上传、校验大小与 Digest、原子发布 Blob，最后提交 Version 元数据。 | 任一步崩溃都不会产生引用半成品的可见 Version。 |
...
| ABI-010 | P0 | Envelope 元数据默认不超过 128 KiB；Header 最多 64 个/总计 32 KiB/单值 8 KiB，Query 最多 128 对/总计 32 KiB，JSON 最大深度 32。 | 所有限制在 SDK Schema 与 Runtime 一致。 |
| ABI-011 | P0 | JSON 重复 Key 必须拒绝；Base64 使用标准带 Padding 编码并拒绝非规范形式。 | 验证器与 Worker 不会对同一输入产生不同解释。 |
...
| ID | 优先级 | 需求 | 验收要点 |
| --- | --- | --- | --- |
| RUN-001 | P0 | 每次调用必须设置端到端墙钟 Deadline；默认 5 秒，管理员硬上限默认 30 秒。Deadline 覆盖排队、下载、验证、编译、实例化、Host API 和 Guest 执行。 | 无限循环 Guest 在 Deadline 加取消宽限期内终止并释放资源。 |
...
| CAP-011 | P0 | Effective Policy 必须绑定不可变 Version/Deployment Generation；Ready Version 的 Capability 不得原地修改。 | 撤权通过新 Generation/Route 完成，旧策略不会重新进入新路由。 |
...
| CAP-015 | P0 | Route Target、Assignment、Endpoint 和 Worker Invocation RPC 必须解析到相同 Admission Epoch、Deployment Generation 与 Effective Policy Digest。 | 任一不匹配返回 `stale_generation`，不以 Guest 字段覆盖。 |
| CAP-016 | P0 | v1.0 的撤权传播上界与 Route+Endpoint LKG 一致，默认最多 5 分钟；需要即时撤权时必须将 `route_max_stale=0` 并切断旧 Endpoint。 | 不宣称分区时即时撤权。 |
...
| WRK-002 | P0 | Controller 必须先在 Worker Registry 中提交 `max_session_epoch+1`，再接受该会话；拒绝低于高水位的状态和 RPC。 | Leader 切换不会复用或降低 Epoch。 |
...
| WRK-005 | P0 | Worker 必须支持 Drain：停止接收新调用，等待有界时间后取消剩余调用并退出。 | Drain 状态可查询且不会获得新放置。 |
| WRK-006 | P0 | Worker 必须周期上报 Replica、活跃 Guest、调用槽、内存、缓存和运行时版本。 | Scheduler 能解释不适配原因。 |
| WRK-007 | P0 | Worker 与 Controller 失联时停止接受新 Assignment/路由更新，但可在 Serving Authorization 有效期内继续当前 Ready Replica。 | 行为是有界 availability-first，不误称无限可用或严格单活。 |
| WRK-008 | P0 | 所有超时与失联判断使用本进程单调经过时间；不得比较跨机器单调 Deadline，也不得在 Raft FSM 中调用 `now()` 决定状态。 | 时钟跳变测试不破坏状态机一致性。 |
| WRK-009 | P0 | Worker 恢复连接时必须先以更高 Session Epoch 注册并提交完整 Inventory；同一 Boot ID 可以重连，但旧 Session/Assignment Generation 不得重新加入 Endpoint。 | 分区愈合不会复活陈旧 Replica。 |
...
| WRK-011 | P0 | Serving Authorization 绑定 Worker/Boot/Session、Assignment Generation、Policy Digest、Discovery Epoch 和 TTL Duration；Worker 以本地单调接收点计时。 | 墙钟与跨机单调时钟不参与比较。 |
...
| SCH-003 | P0 | Scheduler 必须过滤 ABI/Runtime 不兼容、Draining、Unavailable 和资源不足的 Worker。 | 决策解释列出过滤原因。 |
...
| SCH-007 | P0 | 新 Leader 必须先重建 Worker Inventory，并使用稳定窗口避免立即重复调度；窗口内不得发布空 EndpointSnapshot 或创建替代 Assignment，除非收到确定失败证据。 | 健康 Leader 切换不引发副本抖动。 |
...
| SCL-002 | P1 | 扩容信号至少包括正在运行调用数和异步队列深度。 | 两种负载均可触发扩容。 |
| SCL-003 | P1 | 缩容必须有冷却窗口和滞回，避免在阈值附近抖动。 | 合成波动负载不会频繁伸缩。 |
| SCL-004 | P1 | 仅当 `min_replicas=0`、无运行调用、无排队任务且超过 `idle_timeout` 时才能缩容到零。 | 任一条件不满足都不得归零。 |
...
| SCL-010 | P1 | Auto 模式每个评估周期计算 `desired = clamp(ceil(max(inflight, queue_depth) / target_concurrency), min, max)`；扩容立即，缩容需满足 Cooldown/Idle 条件。 | 给定相同指标快照产生相同 Desired，并记录输入与结果。 |
...
| ID | 优先级 | 需求 | 验收要点 |
| --- | --- | --- | --- |
| DSC-001 | P0 | 当前 Leader 必须从已提交 Assignment 与 Ready Observation 生成完整 EndpointSnapshot。 | 未 Ready、陈旧会话或策略摘要不匹配的 Replica 不进入快照。 |
| DSC-002 | P0 | Gateway 必须原子应用 EndpointSnapshot，校验 Discovery Epoch、Sequence、Route Revision 和 Checksum；Epoch/Sequence 断档时全量 Resync。 | 请求不会观察半个 Endpoint 集合。 |
| DSC-003 | P0 | Gateway 可持久化最后有效组合快照用于诊断，但进程重启后不得直接用旧 LKG 服务，必须先从控制面完成一次 Full Sync。 | 无控制面完成同步时 Fail Closed，避免跨进程 TTL 证明错误。 |
| DSC-004 | P0 | Gateway 必须执行被动健康检查，并可配置轻量主动探测；探测只能本地摘除 Endpoint，不能新增。 | 控制面无多数派时停止 Worker 后不再向其发送新请求。 |
| DSC-005 | P0 | 每次 Worker 调用 RPC 必须携带 Admission Epoch、Assignment ID、Boot ID、Session Epoch、Generation 和 Policy Digest。 | Worker 拒绝不匹配的 `stale_assignment`。 |
| DSC-006 | P0 | EndpointSnapshot 与 RouteSnapshot 的最大陈旧窗口必须一致，默认 5 分钟。 | 任一部分过期都不得继续路由。 |
| DSC-007 | P1 | 健康网络下 Endpoint 变化传播到全部健康 Gateway 的 P95 不超过 2 秒。 | 每个 Gateway 暴露 Route Revision、Discovery Epoch 和 Sequence。 |
| DSC-008 | P0 | 无 Quorum 时 Gateway 可删除已失败 Endpoint，但不得生成替代 Endpoint、延长 TTL 或唤醒零副本函数。 | 复合故障不虚构可用性。 |
| DSC-009 | P0 | Gateway 在接收并校验完整 Route+Endpoint 组合快照时记录本地单调接收点；运行期间 TTL 只按单调经过时间计算，墙钟回拨或磁盘诊断快照不得延长。 | TTL 边界可用可控时钟验收。 |
| DSC-010 | P0 | 当前 Leader 只有在确认多数派联络后才能刷新 EndpointSnapshot 与 Serving Authorization；刷新不得延长旧 Discovery Epoch 的已撤销 Generation。 | 无 Quorum 时 Gateway 与 Worker 都在同一上界内停止新工作。 |
| DSC-011 | P0 | Gateway 在合格 Endpoint 中使用 Least-inflight，平局时 Round-robin；本地失败/过载反馈更新被动健康状态。 | 单个慢 Worker 不持续吸收不成比例的新调用。 |
| DSC-012 | P0 | Route 与 Endpoint Watch 可分别到达，但 Gateway 必须暂存，只有对应 Route Revision 和 Generation 都齐全时才原子替换组合快照。 | Endpoint 先到或 Route 先到都不会产生混合配置。 |

#### 10.9.2 Route、灰度与回滚

| ID | 优先级 | 需求 | 验收要点 |
| --- | --- | --- | --- |
| RTE-001 | P0 | Gateway 只能把调用发送到 Ready Version 的有效 Worker 会话。 | Failed/Deprecated 未路由版本不会收到流量。 |
| RTE-002 | P1 | Route 权重使用基点表示，所有有效 Target 权重之和必须为 10000。 | 非法总和在 Raft 提交前拒绝。 |
| RTE-003 | P1 | 分流必须基于稳定哈希；相同 Affinity Key 在同一 Route Revision 下选择相同 Version。 | 重复 100 次结果稳定。 |
| RTE-004 | P1 | 未提供 Affinity Key 时，Gateway 使用 Idempotency Key，否则使用 Request ID。 | 选择过程可通过调试信息解释。 |
| RTE-005 | P0 | Route 更新必须作为一个 Raft 状态机命令原子生效。 | 不出现权重和非 10000 的中间状态。 |
| RTE-006 | P1 | 回滚必须复制目标历史内容并创建新的 Route Revision，不修改历史、不复制或重建 Artifact。 | 新请求只看到旧或新完整快照，在一次控制提交后生效。 |
| RTE-007 | P1 | Route 历史默认保留 7 天或最近 100 个 Route Revision（满足任一淘汰条件后可回收）；每 Function 最多 Pin 20 个。过期且未 Pin 的 Route Revision 不再保证可回滚。 | 历史、Artifact 引用和 GC 都有界。 |
| RTE-008 | P0 | Gateway 必须维护带 Route Revision 的路由缓存，并拒绝同一 Function 的 Route Revision 倒退。 | 乱序更新不会覆盖新配置。 |
| RTE-009 | P0 | 控制面暂时不可达时，Gateway 可从本次进程 Full Sync 后的接收时刻起，使用内存 Last-Known-Good Route+Endpoint 最多 5 分钟；超过上限必须 Fail Closed。 | 重启后未 Full Sync 不使用磁盘旧快照服务。 |
| RTE-010 | P0 | Route Watch Cursor 断档或失效时必须停止增量应用并执行全量 Resync。 | 任一请求只观察旧或新完整快照。 |
| RTE-011 | P1 | 健康网络下，已提交 Route Revision 必须在目标传播时间内到达全部健康 Gateway。 | P95 不超过 2 秒并可观测每个 Gateway Route Revision。 |
| RTE-012 | P0 | 文档必须明确 LKG 窗口内 Route 禁用和权限撤销存在有界延迟；不得同时承诺无限离线可用和即时撤权。 | 故障矩阵、配置和状态页表述一致。 |
...
| RTE-015 | P1 | Route 必须锁定 `hash_version` 和 `salt_id`；所有 Gateway 对相同 Key/Revision 得到相同 Target，算法迁移创建新 Route Revision。 | 混合版本 Gateway 分流一致。 |
...
| SYN-007 | P0 | HTTP Trigger 默认要求独立 Invocation Token；显式 `public` 策略才允许匿名调用。管理 Token 不得转发给 Guest。 | 管理面与调用面权限分离。 |
...
| ID | 优先级 | 需求 | 验收要点 |
| --- | --- | --- | --- |
| ASY-001 | P1 | 异步提交只有在独立耐久队列/任务存储中持久化任务与载荷后才能返回 202；不得写入 Raft。 | 服务重启后已确认任务仍存在。 |
...
| ASY-004 | P1 | Task Store Acquire 必须原子增加 Attempt 并返回不可复用 Lease Token；Start/Renew/Complete/Fail/Cancel 都必须 CAS `{invocation_id, attempt, lease_token, state_revision}`。 | 过期 Attempt 的迟到完成不能覆盖新 Attempt。 |
| ASY-005 | P1 | 默认最多 3 次尝试，使用指数退避和抖动；耗尽后进入 Dead Letter。 | 每次尝试有独立时间和错误。 |
| ASY-006 | P1 | Queued 可确定性取消；Running 为尽力取消；终态不可取消。 | 状态机拒绝非法转换。 |
| ASY-007 | P1 | 默认保存调用元数据与有界结果 24 小时，随后可回收。 | 查询过期结果返回 `result_expired`。 |
...
| ASY-011 | P1 | 去重作用域为 `{principal, namespace, function, idempotency_key}`；Canonical Digest 包含固定 Version、Trigger、业务 Header/Query/Body，排除 Request ID 和观测 Deadline。相同 Key 不同 Digest 返回 409。 | 去重记录覆盖最大任务生命周期和结果保留窗口。 |
| ASY-012 | P1 | Running Lease 到期时，Task Store 按剩余尝试原子进入 RetryScheduled 或 DeadLettered；迟到 Token 只可记录诊断事件。 | E2E 可稳定制造 Late Completion。 |
...
| ASY-014 | P1 | 返回 202 后，按 Invocation ID 查询必须至少返回 Accepted/Queued，不得因投影延迟返回 404；响应携带 Task State Revision 和投影时间。 | Read-after-accept 验收通过。 |
...
| TRG-002 | P1 | Cron 支持标准 5 字段表达式和 IANA 时区，默认 UTC；不存在的夏令时本地时刻跳过，重复本地时刻只在较早一次执行。 | 时区数据库版本锁定，DST Fixture 固定。 |
...
| TRG-007 | P1 | Trigger 更新和禁用必须使用 Resource Revision；已持久化 Invocation 不因禁用而删除。 | 配置与既有任务语义清晰。 |
| TRG-008 | P2 | 可增加外部消息系统适配器，但不属于 v1.0。 | 不影响内置 Event Ingress。 |
| TRG-009 | P1 | `fire_id = trigger_id + scheduled_time + trigger_revision`；Fire Record 与 Invocation 必须在耐久任务存储中原子 Create-if-Absent 或通过 Durable Outbox 收敛，不进入 Raft。 | Leader 切换允许重试但不静默丢失已确认 Fire。 |
| TRG-010 | P1 | Event Ingress 必须要求来源级 Token/签名，且与管理 Token 分离；`public` Event Source 不在 v1.0。 | 未认证事件不进入队列。 |
| TRG-011 | P1 | EventEnvelope 必须包含 `spec_version`、唯一 `event_id`、`source`、`type`、`occurred_at`、`content_type` 和有界 `data`。 | 缺失 ID、超限、重复 Key 或不支持版本被拒绝。 |
| TRG-012 | P1 | Event 去重作用域为 `(trigger_id, event_id, trigger_revision)`；每个匹配 Trigger 创建固定 Version 的独立 Invocation。 | 重试 Event 不重复创建逻辑任务。 |
| TRG-013 | P1 | Event 不保证跨事件或跨 Trigger 顺序；单事件默认最多 Fan-out 32 个 Trigger，超限必须在任何入队前拒绝。 | 不产生部分 Fan-out。 |
| TRG-014 | P1 | Event Ingress 只有在所有匹配 Invocation 持久入队后才返回 202；重试时依靠去重完成未确认结果恢复。 | 队列故障不伪造成功。 |
...
| RFT-003 | P0 | 权威控制读取必须通过 Leader 的 ReadIndex/Quorum Barrier，并等待 `applied_index >= read_index`；只有显式请求的状态聚合可标记为 Stale。 | API 响应包含一致性类型、Revision 和 Applied Index。 |
...
| RFT-025 | P0 | Operation 去重作用域为 `{principal, namespace, operation_id}`，记录 Canonical Request Digest、终态/原响应和 Resource Revision；同 Digest 返回原结果，不同 Digest 返回 409。 | 过期后查询返回 410 `operation_expired`，此后不再保证重复提交去重。 |
...
| `PUT /v1/functions/{name}/deployment` | 更新副本、伸缩和调度策略。 |
...
| `DELETE /v1/triggers/{id}` | 删除无待处理引用的 Trigger。 |
...
| API-002 | P0 | 控制写必须接受持久 Operation ID 和 `If-Match`/Expected Resource Revision；Invocation 按场景接受 Idempotency Key。 | 超时与结果未知时可安全查询或重试。 |
| API-003 | P0 | 分页列表必须使用不透明 Cursor，不使用不稳定页码。 | 并发新增时不会无限重复。 |
| API-004 | P0 | 错误必须包含稳定 `code`、安全 `message`、`request_id` 和可选 `details`。 | 客户端不依赖自然语言判断。 |
| API-005 | P1 | CLI 必须支持 JSON 输出和非交互模式。 | 验收脚本不解析表格文本。 |
...
| API-009 | P0 | 管理写成功响应必须包含 Resource Revision、Raft Applied Index 和 Operation ID。 | 响应可作为提交与 Apply 的证据。 |
...
| RPC-002 | P0 | 内部身份必须来自认证连接或短期签名凭据，不信任用户 Envelope 中的节点、Namespace 或 Function 身份。 | 伪造业务字段不能提升权限。 |
...
| ID | 优先级 | 需求 | 验收要点 |
| --- | --- | --- | --- |
| OBS-001 | P0 | 所有组件必须输出结构化日志，至少包含时间、级别、组件、节点、请求/调用 ID 和错误码。 | 可跨组件关联一次调用。 |
...
| OBS-004 | P0 | 指标必须包括调用总数、状态、延迟、冷/温启动、队列深度、Ready Replica、活跃 Guest、缓存命中、Worker 状态和 Raft 状态。 | 每个核心故障有对应信号。 |
| OBS-005 | P0 | 高基数字段不得直接作为 Metrics Label；Invocation ID 仅进入日志和 Trace。 | 指标基数压力测试可控。 |
| OBS-006 | P0 | 控制写入、权限变更、Route、成员和 Chaos 操作必须产生不可混入普通应用日志的审计事件。 | 可按操作者和资源查询。 |
...
```text
minicloud_invocations_total{function,version,mode,status}
minicloud_invocation_duration_seconds{function,version,mode}
minicloud_cold_start_duration_seconds{stage}
minicloud_async_queue_depth{function}
minicloud_worker_state{worker,state}
minicloud_replica_state{function,version,worker,state}
minicloud_runtime_active_guest_instances{worker}
minicloud_artifact_cache_hits_total{worker}
minicloud_raft_term
...
| CHA-003 | P1 | 外部 Chaos Harness 必须支持停止当前 Leader、停止指定 Worker、通过故障代理丢弃指定 Raft 链路和注入网络延迟。 | 每类动作有自动化验收。 |
| CHA-004 | P1 | 有持续时间的动作由目标进程外的 Harness 自动恢复；Harness 崩溃时必须有租约式恢复保护或明确手工恢复命令。 | 被停止的进程不能承担“自动启动自己”。 |
...
stateDiagram-v2
    ...
    Tombstoned --> [*]: retention elapsed
```

### 11.3 Worker 状态机

`Draining/Removed` 是 Worker Registry 中的控制意图；`Joining/Ready/Suspect/Unavailable/Drained` 是当前 Leader 根据 Session 和 Observation 计算的状态。图描述组合行为，不表示每次 Heartbeat 转换都写入 Raft。
...
stateDiagram-v2
    ...
    Ready --> Draining: admin drain
    Draining --> Drained: calls finish or grace expires
    Drained --> Removed: admin removes worker
    Unavailable --> Joining: same/new boot with higher session epoch
    Removed --> [*]
```
...
stateDiagram-v2
    ...
    Queued --> Cancelled
    Assigned --> Running
    Assigned --> Queued: lease expires before start
...
stateDiagram-v2
    [*] --> Active
    Active --> Draining: scale down or stop requested
...
`Committed/Superseded` 属于控制状态；`Propagating/Effective` 是根据 Gateway Watch Acknowledgement 派生的 Condition，不进入确定性 FSM。
...
stateDiagram-v2
    ...
    Disabled --> Enabled
    Enabled --> Disabled
    Disabled --> [*]: deleted with no pending references
```

Trigger 的 `Healthy/Error` 是派生 Condition。队列故障不得自行改写 `enabled` 配置；管理员修复后无需覆盖原配置即可恢复投递。

状态机实现必须拒绝未在图中或补充转换表中定义的转换，并记录调用者和适用的旧/新 Resource Revision。

取消语义必须单独处理：Queued 状态保证取消；Assigned/Running 只进入 `CancelRequested` 并尽力传播，最终仍可能 Succeeded、Failed 或 TimedOut。API 不得在 Guest 仍可能产生副作用时提前声称 Cancelled。
...
| Leader 故障 | 短暂拒绝/重试，重新选主 | Gateway 使用缓存路由 | 已有函数可继续；控制写短暂不可用 |
| 控制集群失去多数派 | 拒绝写和线性读，不调度、不迁移、不从零激活 | 只有 Gateway LKG 与 Worker Serving Authorization 同时有效的 Ready Replica 可继续；到期后 Worker 不领取新异步任务 | 写返回 `no_quorum`；零副本/无 Endpoint 返回 503；已持久任务等待恢复或过期 |
| Gateway 与控制面分区 | 不影响 Raft | 使用本进程已 Full Sync 的带校验和组合 LKG，最多 5 分钟；被动探活可摘除坏 Endpoint | 响应标注陈旧状态指标；Gateway 重启后必须重新 Full Sync，否则 Fail Closed |
| Worker 心跳丢失 | Leader 标记 Suspect/Unavailable，Connected Gateway 获取新 EndpointSnapshot | Connected Gateway 摘除；与控制面分区的 Gateway 依靠被动探活，可能在 LKG 窗口内短暂命中旧 Endpoint | 同步在途/首次失败可能返回 `worker_lost`；异步租约到期重投 |
| Worker 执行中崩溃 | Reconcile 恢复副本 | 已开始的同步不透明重试 | 同步返回 `worker_lost`；异步至少一次 |
| Worker 与旧 Leader 分区后恢复 | 新 Leader 使用更高 Generation 收敛 | 允许短暂副本重叠，但旧 Endpoint 不重新进入新 Route | 不承诺严格单活；相同异步任务仍以 Invocation ID 去重 |
...
| Observability 后端不可用 | 核心流程继续 | 本地有界缓冲或丢弃并计数 | 不阻塞业务，不静默丢失计数 |
| 时钟跳变 | Raft 安全性不依赖墙钟 | Deadline 使用单调时钟；Cron 按策略去重 | 不产生无限补发或重复调度 |
...
| NFR-PERF-005 | P0 | 所有队列、缓存、日志缓冲和并发均必须有配置上限。 |
...
| ID | 优先级 | 需求 |
| --- | --- | --- |
| NFR-SEC-001 | P0 | 管理 API 至少支持静态管理员 Token；非开发部署必须支持 TLS。 |
...
| NFR-SEC-009 | P0 | Invocation/Event Token 必须使用高熵随机值，只将不可逆校验摘要写入控制状态；明文只在创建时返回一次，比较使用成熟常量时间方法。 |
...
| 配置 | 默认值 | 合法范围/说明 |
| --- | --- | --- |
| Controller voters | 3 | 推荐奇数；v1 最小 3。 |
| Worker heartbeat | 2s | 500ms..30s。 |
| Worker suspect | 6s | 必须大于 heartbeat。 |
| Worker unavailable | 10s | 必须大于 suspect。 |
| Route max stale | 5m | 0 表示控制失联立即 Fail Closed。 |
| Worker serving authorization | 5m | 必须小于等于 Route max stale；0 表示控制失联立即停止新调用。 |
| Serving authorization refresh | 1m | 必须小于 TTL/2，且每次刷新前确认多数派联络。 |
| Route history | 7d / 100 revisions | 显式 Pin 的 Route Revision 不自动回收。 |
| Route pinned revisions | 20 / function | 超限前必须先 Unpin。 |
| Function timeout | 5s | 上限默认 30s；包含完整调用链。 |
| Cancellation grace | 1s | Deadline 后允许 Runtime 清理；不得无限延长。 |
...
| Event data/raw envelope | 1 MiB / 1.5 MiB | JSON 解析前先限制原始字节。 |
...
| Min replicas | 1 | 设置 0 才允许 Scale to Zero。 |
| Max replicas | 10 | 每 Deployment 上限默认不超过 100。 |
| Target concurrency | 10 / replica | 受 Worker 和 Version 并发上限约束。 |
...
| Async task max age | 24h | 超过后进入 Dead Letter/Expired，不再执行。 |
| Cron stale producer window | 5s | 旧 Leader 无法确认多数派后停止 Cron 生产的上界。 |
| Async result TTL | 24h | 1h..30d。 |
| Idempotency TTL | 24h | 不得短于任务重试窗口。 |
| Control Operation TTL | 24h | 持久去重记录不得短于客户端最大重试窗口。 |
...
| E2E-003 | P0 | API | 不可变版本 | Ready Version 修改被拒绝，新发布生成新 Version；删除执行 Tombstone。 |
...
| E2E-008 | P1 | 多进程 | 原子回滚 | 回滚创建新 Route Revision，一次提交恢复旧版本，无 Artifact 重建。 |
| E2E-009 | P1 | 多进程+基准 | Scale to Zero 正向 | 健康、有容量且缓存命中时归零后的首个请求必须成功，激活 P95 不超过 1.5 秒。 |
| E2E-010 | P1 | 组件+集成 | 异步持久化 | 接收 202 后重启队列/任务消费者，Invocation 仍存在，健康 no-op 在 60 秒内进入终态。 |
| E2E-011 | P1 | Chaos | 异步至少一次 | Worker 在 Running 中死亡，任务租约到期后以相同 Invocation/Version 重投，Attempt 可见。 |
| E2E-012 | P1 | 并发 | 幂等提交 | 100 个并发相同 Key+摘要只创建一个逻辑 Invocation；不同摘要返回 409。 |
| E2E-013 | P1 | Chaos | Cron Leader 切换 | 触发边界切换 Leader，Fire ID 去重且符合错过策略。 |
| E2E-014 | P0 | 安全 | Capability 基线 | 固定 WASI Profile 可运行；默认无文件 Preopen、宿主环境不可见、网络不可用，时钟/随机数等锁定 Baseline 正常。 |
| E2E-015 | P0 | 安全+压力 | 资源隔离 | 无限循环、内存增长、大输出和并发日志洪泛均受限，Worker 仍可调用健康函数。 |
| E2E-016 | P0 | 组件+多进程 | Snapshot 恢复 | 触发 Snapshot/Compact，重启全部节点后恢复相同 Applied Index、对象和 State Digest。 |
| E2E-017 | P0 | Chaos | Gateway LKG | 控制失联时组合 LKG 在窗口内继续；超过窗口 Fail Closed；Gateway 重启后未 Full Sync 也 Fail Closed；Epoch/Sequence 不倒退。 |
| E2E-018 | P1 | 安全+Chaos | Chaos 权限 | 默认无法调用；启用后仍需管理员身份，外部 Harness 动作可审计、可恢复。 |
| E2E-019 | P0 | 多进程 | Drain | Worker Drain 后不接收新调用，在途调用在宽限内结束并进入 Drained。 |
| E2E-020 | P0 | 安全 | 平台敏感信息 | 平台生成的日志、Trace、Raft、Snapshot 不自动传播测试 Secret/认证 Header；Guest stderr 单独标识和授权。 |
| E2E-021 | P0 | Chaos+API | 控制写结果未知 | Commit 后响应前断线；同 Operation ID+Digest 返回原结果，不同 Digest 409，TTL 后查询 410。 |
...
| E2E-025 | P0 | Chaos+并发 | 快照原子性 | 丢弃 Route/Endpoint Watch 后全量 Resync；请求只观察旧或新完整组合快照。 |
...
| E2E-032 | P1 | 集成 | Event Ingress | 认证、去重、Fan-out、不保证顺序和全量持久确认行为符合 TRG-010..014。 |
| E2E-033 | P1 | 可观测 | Trace 与决策解释 | 一次同步、异步和冷启动均可关联日志/Trace/指标，并显示 Route、Version、Replica 和阶段耗时。 |
| E2E-034 | P0 | 合同 | API 与 CLI | OpenAPI/Schema、稳定错误、分页、Resource/Route Revision、Operation 查询、JSON CLI 和管理/调用认证均通过合同测试。 |
...
| E2E-038 | P0 | Chaos | LKG/Serving 边界 | Gateway 重启、组合快照或 Worker Serving Authorization 过期均停止新工作；运行中墙钟回拨不延长单调 TTL。 |
| E2E-039 | P1 | 组件+并发 | Async Late Completion | Attempt 1 租约过期、Attempt 2 完成后，Attempt 1 迟到不能覆盖；Cancel/Complete 竞态遵循 CAS。 |
| E2E-040 | P1 | 组件+多进程 | Tombstone 与入队竞态 | 删除与迟到入队并发；所有已返回 202 的任务都能获得固定 Artifact，GC 不提前。 |
...
| E2E-044 | P1 | 模型+集成 | 自动伸缩与滞回 | 重放固定指标快照得到规定 Desired；突发负载立即扩容，阈值抖动不反复缩放，归零满足全部条件。 |
...
退出条件：E2E-001、002、003、014、015、020、027、031 在单节点环境通过。
...
- Gateway Route+Endpoint 组合快照、Epoch/Sequence 和 LKG。
...
- 全链路 Trace、Metrics、审计和决策解释；
- Chaos API 与自动恢复；
- 性能、安全和资源泄漏测试；
- 完整演示脚本与发布证据。

退出条件：E2E-018、033、全部 P1/NFR 和发布证据通过，达到 v1.0。
...
- [x] Worker Heartbeat、Session Epoch、Assignment Generation 与 Reconcile 可恢复；
